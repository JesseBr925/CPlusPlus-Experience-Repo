...
std::string getAPIkey(){
    std::ifstream inputFile("APIInput.txt"); // Open input file
    std::string firstLine; // Declare a string variable to store the first line
    
    if (inputFile.is_open()) { // Check if file is open
        getline(inputFile, firstLine); // Read the first line
        inputFile.close(); // Close input file
        return firstLine;
    }
    else {
        std::cout << "Unable to find API key.  Please review https://api.nasa.gov/ to get a key, and update the input file 'APIInput.txt'." << std::endl;
    }
